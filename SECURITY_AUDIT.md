...
- **Container hygiene (Cat 10):** The `Dockerfile` runs as a non-root user (`USER nextjs`, uid 1001), pins its base image (`node:22-alpine`, not `latest`), and a `.dockerignore` keeps `.git`/`.env`/`node_modules` out of the image. `deploy/k8s.yaml` adds resource requests/limits, `/healthz` probes, and a hardened pod `securityContext`.
